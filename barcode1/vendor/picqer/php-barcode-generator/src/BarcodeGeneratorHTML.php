...
$ppn = $barcodeData->getWidth()/2;
        $html = '<div style="font-size:0;position:relative;width:' . ($ppn * $widthFactor) . 'px;tst:'. $barcodeData->getWidth().';height:' . ($height) . 'px;margin: auto;">' . PHP_EOL;

        $positionHorizontal = 0;
        /** @var BarcodeBar $bar */
        foreach ($barcodeData->getBars() as $bar) {
            $barWidth = round((($bar->getWidth()/2) * $widthFactor), 3);
