...
if($search_type == 'Notifi_counts') {
    ...
    $dta['num'] = $num + $num_row2 + $oorder_task;
    ...
    $num1 = mysqli_query($mysqli, $qry1);
    $num_row11 = mysqli_num_rows($num1);
    
    if($num_row11>0) {

        $data['list'][] = '<li><p class="u text-danger">Order Tasks</p></li>';

        while($task1 = mysqli_fetch_array($num1)) {
            $created = mysqli_fetch_array(mysqli_query($mysqli, "SELECT employee_name, employee_photo FROM employee_detail WHERE id = '". $task1['task_for'] ."'"));
            
            $data['list'][] = '<li> <a onclick="open_orderTask('. $task1['id'] .')"> <img src="'. $created['employee_photo'] .'" alt=""> <h3>'. $task1['activity'] .'</h3> <small>BO: '. sales_order_code($task1['sales_order_id']) .'</small><p style="font-size: 12px;color: #eb8d17;">Task Date : '. date("d M, Y", strtotime($task1['task_date'])) .'</p> </a> </li>';
        }
    }
    
    
    if($num_row1 == 0 && $num_row2 == 0 && $num_row11 == 0) {
...
} else if($search_type == 'startOrderTask') { 
    ...
    if(isset($_POST['id'])) {
        ...
        $in = Insert('order_task_timer', $arr);
        
        $mq = mysqli_insert_id($mysqli);
        
        timeline_history('Insert', 'order_task_timer', $mq, 'Task Timer Started');
        
        $st = array(
            'task_status' => 1,
        );
        
        $in = Update('order_tasks', $st, ' WHERE id = "'. $_REQUEST['id'] .'"');
...
} else if($search_type == 'stopOrderTask') {
    
    $po = mysqli_fetch_array(mysqli_query($mysqli, "SELECT id, start_time, task_id FROM order_task_timer WHERE id = '". $_REQUEST['id'] ."' "));
    ...
    $in = Update('order_task_timer', $arr, ' WHERE id = '. $po['id']);
    
    timeline_history('Update', 'order_task_timer', $_REQUEST['id'], 'Task Timer Stoped');
...
} else if($search_type == 'save_ordertask_comment') {
    ...
    $newName = $type.'_proof_'.rand(10000, 999999). '.' . end($fille);
    ...
    if($type == 'team_task') {
        ...
        $sel = mysqli_fetch_array(mysqli_query($mysqli, "SELECT * FROM order_tasks WHERE id = ". $_REQUEST['id']));
        
        $in = Update('order_tasks', $arr, ' WHERE id = '. $_REQUEST['id']);

        timeline_history('Update', 'order_tasks', $_REQUEST['id'], $sel['activity'] .'Order Task Completed. BO: '. sales_order_code($sel['sales_order_id ']));

        $mk = mysqli_query($mysqli, "SELECT start_time, id FROM order_task_timer WHERE end_time IS NULL AND task_id = ". $_REQUEST['id']);
        while($jk = mysqli_fetch_array($mk)) {
            $ti1 = strtotime(date('Y-m-d H:i:s'));
            $ti2 = strtotime($jk['start_time']);
            $dif = abs($ti2 - $ti1);
            ...
            $in = Update('order_task_timer', $array, ' WHERE id = '. $jk['id']);
        }
    }
    if($in) {
        $data['res'][] = 0;
...
} else if($search_type == 'gatAllorderTaskComments') {
    
   $sqE = mysqli_query($mysqli, "SELECT * FROM common_comments WHERE table_name = 'order_tasks' AND primary_id = '". $_REQUEST['id'] ."' ORDER BY id DESC");
    ...
    <?php
} else if($search_type == 'order_task_sheet') {
    ...
    <div class="modal-header">
        <h5 class="modal-title" id="taskLabel">Order Task</h5>
        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
    </div>
                ...
                <table class="table">
                    <tr>
                        <td style="border: none;">
                            <span class="f-12">Task :</span> <p style="color: #49a2ff;"><?= $task['activity']; ?></p>
                        </td>
                        <td style="border: none;">
                            <span class="f-12">BO :</span> <p style="color: #49a2ff;"><?= sales_order_code($task['sales_order_id']); ?></p>
                        </td>
                        <td style="border: none;" class="float-right" id="timerButton_ord">
                            <?php
                                ...
                                if($H==1) {
                            ?>
                                <a class="btn btn-outline-danger timer_order" onclick="stopOrderTask(<?= $ftch['id']; ?>)"><i class="icon-copy fa fa-clock-o" aria-hidden="true"></i> Stop Timer</a>
                            <?php } else { ?>
                                <a class="btn btn-outline-success timer_order" onclick="startOrderTask(<?= $task['id']; ?>)"><i class="icon-copy fa fa-clock-o" aria-hidden="true"></i> Start Timer</a>
                            <?php } } ?>
                        </td>
                    </tr>
                </table>
                    ...
                    <div class="">
                        ...
                        <br>
                            <?php if($task['task_status'] != 2) { ?>
                                <textarea class="form-control" name="team_task_comment" id="order_task_comment" placeholder="Add Comment" style="height:90px"></textarea>
                                <div style="padding: 5px;text-align: right;">
                                    <a class="btn btn-outline-primary cmtBtn_ord d-none" onclick="save_ordertask_comment(<?= $task['id']; ?>)">Save Comment</a>
                                </div>
                            <?php } ?>
                        <hr>
                        <br>
                        
                        <div id="addedComments_ord">
                            <?php
                                $sqE = mysqli_query($mysqli, "SELECT * FROM common_comments WHERE table_name = 'order_tasks' AND primary_id = '". $task['id'] ."' ORDER BY id DESC");
                            ...
                            ?>
                                ...
                                <hr>
                            <?php } } else { print '<p class="text-center">No Comments Found!</p>'; } ?>
                        </div>
                    </div>
                </div>
                <div class="col-md-4" style="background: #f0f5f7;">
                    ...
                    <span class="f-12">Created date <span style="color:#17a2b8"><?= $created['employee_name']; ?></span> <?= date('d-m-Y H:i A', strtotime($task['created_date'])); ?></span>
                    
                    <table class="table f-12">
                        <tr>
                            <td><i class="fa fa-star-half-o pull-left fa-fw fa-lg"></i>Status</td>
                            <td id="modTaskStatus_ord" style="color:<?= $status_color[$task['task_status']]; ?>"><?= $status[$task['task_status']]; ?></td>
                        </tr>
                        <tr>
                            <td><i class="fa fa-fw fa-lg fa-calendar-plus-o pull-left fa-margin"></i>Task Date</td>
                            <td><?= date('d M Y', strtotime($task['task_date'])); ?></td>
                        </tr>
                        <tr>
                            <td>Allowed Working Time</td>
                            <td><?= time_calculator_new(time_calculator($task['task_timeing']), 1); ?></td>
                        </tr>
                        <tr>
                            <td style="border-bottom: 1px solid #dee2e6;"><i class="fa fa-asterisk fa-fw fa-lg"></i>Time Taken</td>
                                <?php
                                    $running1 = mysqli_fetch_array(mysqli_query($mysqli, "SELECT start_time FROM order_task_timer WHERE task_id='". $task['id'] ."' AND employee_id = '". $logUser ."' AND end_time IS NULL"));
                                    ...
                                    $stoped1 = mysqli_fetch_array(mysqli_query($mysqli, "SELECT sum(total_time) as total_time FROM order_task_timer WHERE task_id='". $task['id'] ."' AND employee_id = '". $logUser ."'"));
                    ...
                    <p class="u"><i class="fa fa-user-o" aria-hidden="true"></i> Followers :</p>
                        <div style="text-align: center !important;">
                            <?php
                                $sq = mysqli_query($mysqli, "SELECT * FROM employee_detail WHERE id IN (". $task['resp_b'] .") "); 
                                while($foll = mysqli_fetch_array($sq)){
                                    print viewImage($foll['employee_photo'], 50);
                                }
                            ?>
                        </div>
                    <hr>
                    ...
                    <?php } if($task['task_status']==1) { ?>
                        <form id="ImageForm" onsubmit="return false">
                            <input type="file" class="form-control" style="border:none; background-color: #f0f5f7;" name="proof_image_order_task" id="proof_image_order_task">
                            <br>
                            <button style="position: relative; left:20%;" type="button" class="btn btn-outline-info compl" onclick="markAsComplete(<?= $task['id']; ?>, 'order_task')"><i class="icon-copy ion-ios-checkmark-outline"></i> Mark As Complete</button>
                        </form>
                    
                    <?php } else if($task['task_status']==0) { print '<p class="text-center"><small>Task Not Started!</small></p>'; } else { print '<p class="text-center" style="color:'. $status_color[$task['task_status']] .'"><small>Task '. $status[$task['task_status']] .'!</small></p>'; print '<a href="download.php?f='. $task['task_proof'] .'" class="f-12" style="color:#a5a5a5"><i class="icon-copy fa fa-cloud-download" aria-hidden="true"></i> Download Proof</a>'; } ?>
                        <br>
                </div>
            </div>
        </div>
    ...
    <script>
        $(document).ready(function(){
            ...
            $('#order_task_comment').on('keyup', function() {
    ...
    <?php
} else if($search_type == 'openTaskSheet_OnlyView') {
