        ...
        <div class="pd-ltr-20 xs-pd-20-10">
            <div class="min-height-200px">
                ...
                <div class="card-box mb-30">
                    ...
                    <div class="pd-20">
                        <h4 class="text-blue h4">Garment Process Output</h4>
                    </div>
                    <div class="pb-20">
                        <table class="data-table table stripe hover nowrap dataTable no-footer dtr-inline collapsed">
                            ...
                            <tbody>
                                ...
                                $qry .= " FROM orbidx_component_process a ";
                                ...
                                while ($sql = mysqli_fetch_array($query)) {
                                    ...
                                    <tr>
                                        <td><?= $x; ?></td>
                                        <td><?= employee_name_from_code($sql['device_user']); ?></td>
                                        <td><?= date('d-M, Y', strtotime($sql['date'])); ?></td>
                                        <td><?= process_name($sql['process']); ?></td>
                                        ...
                                        <td><?php foreach(explode(',', $sql['piece_id']) as $key => $pcs) { $pcs = explode('-', $pcs); print ($key==0) ? $pcs[3] : ', '.$pcs[3]; } ?></td>
                                    </tr>
                                    <?php $x++;
                                } ?>
                                <?php
                                $qry = "SELECT a.*, b.bundle_number ";
                                $qry .= " FROM orbidx_garment_process a ";
                                $qry .= " LEFT JOIN bundle_details b ON a.bundle_details_id = b.id ";
                                $qry .= " ORDER BY a.id DESC ";
