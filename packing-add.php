...
if (isset($_POST['SaveBtn'])) {
    ...
    if ($_POST['iid'] == "") {
        ...
        $ins = Update('packing', $value2, 'WHERE id = ' . $_POST['iid']);
        $inid = $_POST['iid'];
    }
    ...
    for ($p = 0; $p < count($_POST['sod_size']); $p++) {
        ...
        $val = array(
            ...
            'packing_qty' => $_POST['packing_qty'][$p],
        );

        if ($_POST['packing_det_id'][$p] == '') {
            $ins = Insert('packing_detail', $val);
        } else {
            ...
            $ins = Update('packing_detail', $val1, 'WHERE id = ' . $_POST['packing_det_id'][$p]);
        }
    }
...
if (isset($_GET['id'])) {
    $ID = $_GET['id'];
    $comp = 'Edit Packing';
    $sql = mysqli_fetch_array(mysqli_query($mysqli, "SELECT * FROM packing WHERE id=" . $ID));
} else {
    $comp = 'Add Packing';
    $ID = '';
}
...
<head>
    <!-- Basic Page Info -->
    <meta charset="utf-8">
    <title>BENSO GARMENTING - Packing
    </title>
        ...
        <div class="pd-ltr-20 xs-pd-20-10">
            <div class="min-height-200px">
                ...
                <div class="pd-20 card-box mb-30">
                    ...
                    <div class="pd-20">
                        <a class="btn btn-outline-primary" href="packing.php" style="float: right;"><i
                                class="fa fa-list" aria-hidden="true"></i> Packing List</a>
                    </div>
                    <div class="clearfix">
                        <div class="pull-left">
                    ...
                    <form id="add-process" method="post" autocomplete="off">
                        ...
                        <div class="row">
                            ...
                            if (isset($_GET['id'])) {
                                ...
                                $qryz = mysqli_query($mysqli, "SELECT * FROM packing WHERE entry_number LIKE '%IR-%' ORDER BY id DESC");
                            ...
                            <div class="col-md-12">
                                <table class="table table-bordered">
                                    ...
                                    <tbody id="main_tbody">
                                        <?php
                                        if (isset($_GET['id'])) {
                                            $da = mysqli_query($mysqli, "SELECT * FROM packing_detail WHERE packing_id = " . $ID);
                                            if (mysqli_num_rows($da) > 0) {
                                                $sl = 1;
                                                while ($result = mysqli_fetch_array($da)) {
                                                    $sum = mysqli_fetch_array(mysqli_query($mysqli, "SELECT sum(packing_qty) as packing_qty FROM packing_detail WHERE sod_size = '" . $result['sod_size'] . "' AND variation_value = '" . $result['variation_value'] . "'"));
                                                    print '<tr>
                                                            <td>' . $sl . '</td>
                                                    ...
                                                    if ($_GET['type'] == 'view') {
                                                        ...
                                                        print '<td>' . $result['packing_qty'] . '</td>';
                                                    } else {
                                                        print '<td>
                                                                    <input type="hidden" name="packing_det_id[]" value="' . $result['id'] . '">
                                                                    <input type="hidden" name="sod_size[]" value="' . $result['variation_value'] . '">
                                                                    <input type="text" name="packing_qty[]" style="max-width: 250px;" class="form-control" placeholder="packing Qty" value="' . $result['packing_qty'] . '"></td>';
                                                    }
                                                    print '</tr>';
                                                    $sl++;
                                                }
    ...
    <script>
        $("#sod_part").change(function () {
            ...
            if (id != "") {
                $.ajax({
                    type: 'POST',
                    url: 'ajax_search3.php?type=sizeWizeListfor_packing',
                    data: data,
                    success: function (msg) {
                        var json = $.parseJSON(msg);
