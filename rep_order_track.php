...
<head>
    <!-- Basic Page Info -->
    <meta charset="utf-8">
    <title>BENSO GARMENTING - Order Tracking Report</title>
...
<body>
        ...
        <div class="pd-ltr-20 xs-pd-20-10">
            <div class="min-height-200px">
                ...
                <div class="card-box mb-30">
                    <?php if(RP_ORDER_TRACKING!=1) { action_denied(); exit; } ?>
                    <div class="pd-20">
                        <div class="row">
                            <div class="col-md-3">
                            ...
                            <div class="col-md-3">
                                ...
                                <input value="Clear" type="button"
                                    onclick="window.location.href='rep_order_track.php?fdt=<?= date('Y-m-01'); ?>&tdt=<?= date('Y-m-t'); ?>'"
                ...
                <script>
                    function startFilter() {
                        ...
                        window.location.href = "rep_order_track.php" + search;
                    }
                ...
                <div class="card-box mb-30" id="divToPrint">
                    <div class="pd-20" style="text-align:center">
                        <h4 class="text-blue h4">Order Tracking Report</h4>
                    </div>
                    <div class="pb-20">
                        <table class="table table-striped">
                            ...
                            <tbody>
                                ...
                                while ($sql = mysqli_fetch_array($query)) {
                                    ...
                                    $iron = mysqli_fetch_array(mysqli_query($mysqli, "SELECT sum(a.ironing_qty) as ironing_qty FROM ironing_detail a WHERE a.style_id='" . $sql['id'] . "' AND part_id = '". $sql['part'] ."'"));
                                    
                                    $pack = mysqli_fetch_array(mysqli_query($mysqli, "SELECT sum(a.packing_qty) as packing_qty FROM packing_detail a WHERE a.style_id='" . $sql['id'] . "' AND part_id = '". $sql['part'] ."'"));
                                    
                                    $cmt = mysqli_fetch_array(mysqli_query($mysqli, "SELECT a.*,b.employee_name FROM common_comments a LEFT JOIN employee_detail b ON a.employee_id = b.id WHERE a.table_name='sales_order_detalis' AND a.primary_id='" . $sql['id'] . "' AND part_id = '". $sql['part'] ."' ORDER BY id DESC"));
                                    ...
                                    <tr>
                                        ...
                                        <td><?= $iron['ironing_qty'] ? $iron['ironing_qty'] : 0; ?></td>
                                        <td><?= $pack['packing_qty'] ? $pack['packing_qty'] : 0; ?></td>
                                        <td>-</td>
                                        <td>-</td>
                                        <td>
                                            <i class="icon-copy ion-plus-round 1conA1<?= $x; ?>" onclick="addIcon(<?= $x; ?>)"></i> &nbsp;
                                            
                                            <span><textarea class="form-control dhide 1conA2<?= $x; ?>" id="comment<?= $x; ?>" style="height: 50px;"></textarea></span>
                                            <span class="1conA1<?= $x; ?> txt<?= $x; ?>"  title="By : <?= $cmt['employee_name']; ?>"> <?= $cmt['comment']; ?></span>
                                            
                                            <i class="icon-copy ion-checkmark dhide 1conA2<?= $x; ?>" onclick="saveIcon(<?= $x.','.$sql['id'].','.$sql['part']; ?>)"></i>&nbsp;&nbsp;&nbsp;&nbsp;
                                            <i class="icon-copy ion-close-round dhide 1conA2<?= $x; ?>" onclick="cancelIcon(<?= $x; ?>)"></i>
                                        </td>
                                    </tr> 
                                    <?php $x++;
                                ...
                                <tr class="d-none">
                                    <td colspan="<?= ($_GET['ig'] == 'true') ? '11' : '10'; ?>"
                                        style="text-align:right">
                                        <b>Cutting Production Total :</b>
                                    </td>
                                    <td colspan="2">
                                        <b>
                                            <?= array_sum($CPT); ?>
    ...
    <script>
        ...
        function saveIcon(e, id, part)
        {
            var cmt = $("#comment"+e).val();
            
            $.ajax({
                type:'POST',
                url:'ajax_action.php?common_comment=1&id='+id+'&part='+ part + '&comment='+ cmt +'&table=sales_order_detalis',
